...
"\n"
"options:\n"
"-l1 <lambda1>: set L1 regularization parameter (default 0.00002)\n"
"-l2 <lambda2>: set L2 regularization parameter (default 0.00002)\n"
"-k <factor>: set number of latent factors (default 4)\n"
"-t <iteration>: set number of iterations (default 15)\n"
"-r <alfa>: set learning rate for ftrl(equal to alfa) (default 0.2)\n"
"-b <beta>: set parameter for ftrl(equal to beta) (default 0.1)\n"
...
"--no-linear: without linear term in FFM. (default false)");
}
...
Option parse_option(int argc, char **argv)
{
	...
	//cout <<argc << " " << i <<" " <<args[i] << endl;
        ...
        {
            if(i == argc-1)
                throw invalid_argument("need to specify alfa(in ftrl) after -r");
            i++;
            opt.param.alfa = atof(args[i].c_str());
            if(opt.param.alfa <= 0)
        ...
        {
            if(i == argc-1)
                throw invalid_argument("need to specify beta(in ftrl) after -b");
            i++;
            opt.param.beta = atof(args[i].c_str());
            if(opt.param.beta < 0)
                throw invalid_argument("parameter beta included in ftrl should be greater than zero");
        }
        else if(args[i].compare("-l1") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify lambda1 after -l1");
            i++;
            opt.param.lambda1 = atof(args[i].c_str());
            if(opt.param.lambda1 < 0)
                throw invalid_argument("regularization cost l1 should not be smaller than zero");
        }
        else if(args[i].compare("-l2") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify lambda2 after -l2");
            i++;
            opt.param.lambda2 = atof(args[i].c_str());
            if(opt.param.lambda2 < 0)
                throw invalid_argument("regularization cost l2 should not be smaller than zero");
        }
        ...
        else if(args[i].compare("--no-linear") == 0)
        {
            opt.param.no_linear = true;
        }
        else
        {
            break;
