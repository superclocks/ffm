...
{
    ffm_float alfa = param.alfa;
    ffm_float beta = param.beta;
    ffm_float lambda1 = param.lambda1;
    ffm_float lambda2 = param.lambda2;
    bool no_linear = param.no_linear;
    ...
    if(do_update)
    {
        ffm_float* bias = model.bias;
        ffm_float* ni = model.bias + 1;
        ffm_float* zi = model.bias + 2;
        ffm_float g_bias = kappa * 1 / sqrt(r); // 第j个特征上的梯度
        ffm_float sigma_bias = (sqrt(*ni + g_bias * g_bias) - sqrt(*ni)) / alfa;
        *zi += g_bias - sigma_bias * (*bias);
        *ni += g_bias * g_bias;
        *bias = -1.0 / ((beta + sqrt(*ni)) / alfa + lambda2) * (*zi - sign(*zi) * lambda1);
    }
    //训练线性项
    ffm_float wTx0 = 0.0;
    if(!no_linear)
    {
        if(do_update)
        {
            ffm_float* w0 = model.W0;
            ffm_float* n_j = w0 + model.n;
            ffm_float* z_j = w0 + model.n * 2;
            for(ffm_node* N1 = begin; N1 != end; N1++)
            {
                ...
                if(j > model.n)
                    continue;
                ffm_float g_j = kappa * v / sqrt(r); // 第j个特征上的梯度
                ffm_float sigma_j = (sqrtf(*(n_j + j) + g_j * g_j) - sqrtf(*(n_j + j))) / alfa;
                *(z_j + j) += g_j - sigma_j * (*(w0 + j));
                *(n_j + j) += g_j * g_j;
                if(fabs(*(z_j + j)) <= lambda1)
                    *(w0 + j) = 0.0;
                else
                    *(w0 + j) = - 1.0 / ((beta + sqrtf(*(n_j + j))) / alfa + lambda2) *
                    (*(z_j + j) - sign(*(z_j + j)) * lambda1);
            }
        }
        else
        {
    ...
    return t + wTx0 + model.bias[0]; //wTx0为线性项，如果没有线性项则为0
}
...
ffm_model* init_model(ffm_int n, ffm_int m, ffm_parameter param)
{
    ...
    try
    {
        model->bias = malloc_aligned_float((ffm_float)3);
    }
    ...
    model->bias[0] = coef*distribution(generator);
    model->bias[1] = 0;
    model->bias[2] = 0;
    //--------------------初始化线性项--------------------------
    if(!param.no_linear)
    {
        try
        {
            model->W0 = malloc_aligned_float((ffm_float)n * 3);
        }
        catch(bad_alloc const &e)
        {
            ffm_destroy_model(&model);
        ...
        ffm_float* w0 = model->W0;
        for(ffm_int d = 0; d < model->n; d++, w0++)
            *w0 = coef*distribution(generator);
        for(ffm_int d = model->n; d < model->n * 2; d++, w0++)
            *w0 = 1;
        for(ffm_int d = model->n * 2; d < model->n * 3; d++, w0++)
            *w0 = 1;
...
#pragma omp parallel for schedule(static) reduction(+: tr_loss)
        ...
        for(ffm_int ii = 0; ii < tr->l; ii++)
        {
            ...
            ffm_float r = R_tr[i];
            ffm_float t = wTx(begin, end, r, *model, param);
            ffm_float expnyt = exp(-y*t);
            tr_loss += log(1+expnyt);
            ffm_float kappa = -y*expnyt/(1+expnyt);
            wTx(begin, end, r, *model, param, kappa, true);
        }
...
ffm_int ffm_save_model(ffm_model *model, char const *path)
{
    ...
    //保存偏置项
    f_out << "bias " << model->bias[0] << "\n";
    cout << "偏置"<< endl;
    //保存线性项
    if(model->W0 != nullptr)
    {
        f_out << "w ";
        for(ffm_int i = 0; i < model->n; i++)
            f_out << model->W0[i] << " ";
        f_out << "\n";
    }
    cout << "线性项" << endl;
...
ffm_model* ffm_load_model(char const *path)
{
    ...
    try
    {
        model->bias = malloc_aligned_float((ffm_long)1);
    }
    catch(bad_alloc const &e)
    {
        ffm_destroy_model(&model);
    ...
    try
    {
        model->W0 = malloc_aligned_float((ffm_long)model->n);
    }
    catch(bad_alloc const &e)
    {
        ffm_destroy_model(&model);
...
void ffm_destroy_model(ffm_model **model)
{
    if(model == nullptr || *model == nullptr)
        return;
#ifdef _WIN32
    _aligned_free((*model)->bias); //free偏置
    _aligned_free((*model)->W0); //free线性项
    _aligned_free((*model)->W); //free交叉项
#else
    free((*model)->bias); //free偏置
    free((*model)->W0); //free线性项
    free((*model)->W); //free交叉项目
#endif
    delete *model;
    *model = nullptr;
}
...
ffm_model* ffm_train_with_validation(ffm_problem *tr, ffm_problem *va, ffm_parameter param)
{
    ...
    model_ret->bias = model->bias;
    model_ret->W0 = model->W0;
    model_ret->W = model->W;
...
ffm_float ffm_predict(ffm_node *begin, ffm_node *end, ffm_model *model)
{
    ...
    return 1/(1+exp(-(t + t0 + model->bias[0])));
}
